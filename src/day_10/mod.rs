...
pub fn total_signal_strength(input_file: &str) -> Result<i32, std::io::Error> {
    ...
    let signal_strength = input.lines().fold(0i32, |mut acc, line| {
        ...
        match instruction {
            Instruction::AddX(n) => {
                for _ in 0..2 {
                    acc = add_signal_strength(&cpu, acc);
                    cpu.num_cycles += 1;
                }
...
pub fn render_image(input_file: &str, spacing: Option<usize>) -> Result<String, std::io::Error> {
    let input = read_to_string(input_file)?;
        ...
        match instruction {
            Instruction::AddX(n) => {
                for _ in 0..2 {
                    ...
                    draw(&mut screen, crt.register_x, &i, &j);
                    crt.num_cycles += 1;
                    j += 1;
                }
                crt.register_x += n;
    ...
    Ok(print_screen(screen, spacing))
}

fn print_screen(screen: Vec<Vec<char>>, spacing: Option<usize>) -> String {
    let mut render = String::from("");
    let space = spacing.unwrap_or_default();
    for row in screen {
        for pixel in row {
            render.push(pixel);
            for _ in 0..space {
...
fn draw(screen: &mut Vec<Vec<char>>, register_x: i32, i: &usize, j: &usize) {
    if (register_x - *j as i32).abs() < 2 {
        screen[*i][*j] = '#';
    }
}
...
fn add_signal_strength(cpu: &CPU, strength: i32) -> i32 {
...
mod test_super {
    ...
    fn test_image_rendering() {
        assert_eq!(
            render_image("src/day_10/example_input.txt", None).unwrap(),
            [
                "##..##..##..##..##..##..##..##..##..##..\n",
                "###...###...###...###...###...###...###.\n",
                "####....####....####....####....####....\n",
